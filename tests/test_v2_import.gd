...
func _run() -> void:
	...
	fixture.control.state_changed.connect(func(online: bool, _starting: bool, _pid: int, message: String) -> void:
		...
		core_online = online
		if online:
			fixture.control.refresh_runtime()
	)
	fixture.control.api_result.connect(func(action: String, ok: bool, payload: Variant) -> void:
		if action == "proxies" and ok and payload is Dictionary:
			proxy_payload = payload
	)
	var profile_before_invalid := FileAccess.get_file_as_string(subscription.profile_path())
	if subscription.use_v2_share_links("vless://broken"):
		printerr("FAIL: 损坏的 V2 链接未被拒绝")
		quit(4)
		return
	if FileAccess.get_file_as_string(subscription.profile_path()) != profile_before_invalid:
	...
	if not subscription.use_v2_share_links(links):
	...
	if not subscription.use_v2_share_links(base64_subscription):
