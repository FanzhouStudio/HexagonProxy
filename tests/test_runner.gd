...
func _init() -> void:
	...
	assert(controller.CORE_RECOVERY_DELAYS == [1.0, 3.0, 8.0])
	...
	assert(controller.GENERATED_ROUTING_RULES.contains("IP-CIDR,127.0.0.0/8,DIRECT,no-resolve"))
	assert(controller.GENERATED_ROUTING_RULES.contains("IP-CIDR,192.168.0.0/16,DIRECT,no-resolve"))
	assert(controller.GENERATED_ROUTING_RULES.contains("IP-CIDR6,::1/128,DIRECT,no-resolve"))
	assert(controller.GENERATED_ROUTING_RULES.contains("RULE-SET,hexagon-cn-domain,DIRECT"))
	assert(controller.GENERATED_ROUTING_RULES.contains("RULE-SET,hexagon-cn-ip,DIRECT,no-resolve"))
	assert(controller.GENERATED_ROUTING_RULES.ends_with("MATCH,六角选择\n"))
	assert(controller.GENERATED_RULE_PROVIDERS.contains("format: mrs"))
	assert(FileAccess.file_exists(controller.runtime_dir().path_join("rules/geosite-cn.mrs")))
	assert(FileAccess.file_exists(controller.runtime_dir().path_join("rules/geoip-cn.mrs")))
	...
	assert(bool(controller._validate_active_profile().get("ok", false)))
