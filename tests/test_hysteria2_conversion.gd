...
func _run() -> void:
	...
	if not subscription.use_v2_share_links(links):
		...
		return
	var entry: Dictionary = subscription.get_subscriptions().back()
	...
	if regular_file.is_empty() or hy2_file.is_empty():
		...
		return
	var regular_path: String = subscription.subscription_provider_dir().path_join(regular_file)
	var hy2_path: String = subscription.subscription_provider_dir().path_join(hy2_file)
	...
	await process_frame
	subscription.initialize()
	entry = subscription.get_subscriptions().back()
