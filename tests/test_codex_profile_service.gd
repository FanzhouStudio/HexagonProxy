...
func _run() -> void:
	...
	var reauthorized: Dictionary = await authorization.authorize_profile("重新授权")
	check(bool(reauthorized.get("ok")) and authorization.profiles().size() == 2, "same identity reauthorization updates existing account")
	...
	reopened.queue_free()
	var deletion = new_service("deletion")
	deletion.account_store.update("default", deletion.live_info)
	var replacement: Dictionary = await deletion.create_profile("保留账号")
	var replacement_id := str(replacement["profile_id"])
	deletion.account_store.update(replacement_id, {"account_id": "replacement-id", "email": "replacement@example.invalid"})
	check(bool((await deletion.switch_profile(replacement_id)).get("ok")), "switches away before deleting default")
	check(bool(deletion.forget_profile("default").get("ok")), "inactive default account can be removed")
	check(deletion.account_store.find("default").is_empty(), "deleted default leaves account list")
	var deletion_reopened = new_service("deletion")
	check(deletion_reopened.account_store.find("default").is_empty(), "deleted default is not recreated on restart")
	check(not bool(deletion_reopened.forget_profile(replacement_id).get("ok")), "last account cannot be removed")
	deletion.queue_free()
	deletion_reopened.queue_free()
	var duplicate_capture = new_service("duplicate_capture")
	duplicate_capture.account_store.update("default", duplicate_capture.live_info)
	var empty_row: Dictionary = await duplicate_capture.create_profile("空账号")
	var duplicate_result: Dictionary = await duplicate_capture.capture_current("", str(empty_row["profile_id"]))
	check(not bool(duplicate_result.get("ok")) and duplicate_capture.profiles().size() == 2, "same identity cannot fill another account row")
	duplicate_capture.queue_free()
	var duplicate_migration = new_service("duplicate_migration")
	duplicate_migration.account_store.update("default", duplicate_migration.live_info)
	var duplicate_row: Dictionary = await duplicate_migration.create_profile("重复账号")
	var duplicate_id := str(duplicate_row["profile_id"])
	duplicate_migration.account_store.update(duplicate_id, duplicate_migration.live_info)
	check(bool((await duplicate_migration.switch_profile(duplicate_id)).get("ok")), "duplicate fixture selects newer row")
	var duplicate_reopened = new_service("duplicate_migration")
	check(duplicate_reopened.profiles().size() == 1 and duplicate_reopened.selected_id() == duplicate_id, "legacy duplicate identities collapse to selected row")
	duplicate_migration.queue_free()
	duplicate_reopened.queue_free()
	...
	check(migrated.profiles().size() == 1 and migrated.selected_id() == "legacy", "legacy list and selection retained without recreating default")
